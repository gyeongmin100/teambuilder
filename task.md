...
## 23. 2026-02-22 데이터 가져오기 기능 테이블 통합
- [x] 독립된 데이터 가져오기 섹션 제거
- [x] 참가자 테이블 툴바에 데이터 가져오기 토글 버튼 추가
- [x] 구글폼 URL/불러오기/내 구글폼/CSV 업로드를 테이블 카드 내부 패널로 이동
- [x] 폼 목록(sheetListOpen) 렌더링을 테이블 카드 내부로 이동
- [x] 빌드 검증 통과 (`npm run build`)

## 24. 2026-02-22 입력 하단 실행 순서/섹션 번호 정리
- [x] 실행 전 점검 요약 카드를 결제 실행 버튼 위로 이동
- [x] 결제 후 팀 배정 실행 버튼을 입력 카드 최하단으로 이동
- [x] 기능 섹션 제목 앞 번호(예: 1), 4)) 제거
- [x] 빌드 검증 통과 (`npm run build`)

## 25. 2026-02-22 입력페이지 UX 전면 정리(요청 10개 반영)
- [x] `한 페이지에서 바로 설정 후 실행` 문구 제거
- [x] 검색 입력 위치를 테이블 헤더 상단(`No` 위)으로 이동
- [x] 열 제외 체크 UI 제거, 열 헤더에서 직접 삭제 가능한 방식으로 전환
- [x] 열 이름 수정 가능성을 헤더 액션(`이름변경`) + 안내문구로 명확화
- [x] 핀 아이콘 제거, 기준 열 드롭다운 선택 방식으로 전환
- [x] 1팀당 인원 입력의 4 고정 체감 문제 해소(강제 fallback 제거)
- [x] 나머지 처리 옵션을 나머지 발생 시에만 노출
- [x] 실행 전 점검 요약에서 행동 메시지 노출 제거
- [x] 실행 전 점검 요약 항목을 요구 스펙(데이터수/기준열/중복값/팀수/팀구성/프롬프트 사용여부)으로 교체
- [x] 실행 버튼 외곽 박스 제거 + 우측 하단 배치
- [x] 빌드 검증 통과 (`npm run build`)

## 26. 2026-02-22 입력 UX 보완 7개 반영(재요청)
- [x] 외부데이터 가져오기 패널 기본 닫힘
- [x] 외부데이터 가져오기 버튼을 `참가자 데이터(테이블)` 제목 우측으로 이동
- [x] 행추가/열추가 버튼을 검색 입력 옆으로 이동
- [x] 열 헤더의 `이름변경` 텍스트 버튼 제거
- [x] 열 더블클릭 편집 + 포커스 아웃(다른 영역 클릭) 저장 적용
- [x] 1팀당 인원 입력을 자유 편집 가능하도록 개선(백스페이스 허용)
- [x] 맞춤프롬프트 활성화 토글 추가 + 실행요약/실행payload를 실제 활성 조건(토글 ON + 텍스트 있음)으로 연동
- [x] 빌드 검증 통과 (`npm run build`)

## 27. 2026-02-22 입력페이지 추가 수정(안내문구 제거/팀인원 기본값 제거/프롬프트 로직 단순화)
- [x] 기준열/열이름수정 안내 문구 2줄 제거
- [x] 1팀당 인원 입력 기본 표시값 제거(빈 입력으로 시작)
- [x] 사용자 맞춤 프롬프트 사용 토글 UI 제거
- [x] 프롬프트 입력값 존재 여부만으로 사용/미사용 판정 로직 정리
- [x] 팀인원 미입력/2 미만 시 실행 가드 추가
- [x] 빌드 검증 통과 (`npm run build`)

## 28. 2026-02-22 외부데이터 패널/열삭제/기준열 초기화/팀인원 입력 개선
- [x] 외부데이터 패널을 버튼 바로 아래로 이동(검색 행 위)
- [x] 외부데이터 안내문구에서 `빈 행 추가` 제거
- [x] 열 삭제 버튼을 텍스트에서 소형 휴지통 아이콘으로 변경
- [x] 기준 열 선택 영역 우측에 조건부 `초기화` 버튼 추가(데이터 있을 때만 표시)
- [x] 팀인원 입력을 1~참가자수로 제한하고 Enter 입력 확정 지원
- [x] 식별열 자동 강제 선택 로직 보정(초기화 시 빈값 유지)
- [x] 팀인원 실행 가드 문구를 1 이상 기준으로 수정
- [x] 빌드 검증 통과 (`npm run build`)

## 29. 2026-02-22 언어 토글 1회 미적용 버그 수정
- [x] 한/영 전환 시 1회 클릭으로 반영되지 않는 레이스 원인 분석
...
## 33. 2026-02-22 결과 페이지 정합성/가독성/기능 확장 (요청 6개 반영)
- [x] spread 모드 나머지 분배를 균등 분배로 강제(랜덤 몰림 제거)
- [x] AI 정규화 후처리에서도 spread 목표 팀 크기로 재균등화 적용
- [x] 결과 요약에서 내부 기술 문구(remainderMode/파싱소스/제약 판정 로그) 제거
- [x] 팀별 사유 문장 강화 + 성비 불가피 상황 자동 멘트 추가
- [x] 결과 페이지 `이미지로 저장`, `공유하기` 기능 추가
- [x] `프롬프트 수정 후 다시 배정` 버튼 제거
- [x] 구성원 클릭 시 특성 상세 펼침 UI 추가
- [x] 분배 시나리오 검증(10/4, 14/4, 13/4 spread, 13/4 keep_partial) 통과
- [x] 빌드 검증 통과 (`npm run build`)

## 34. 2026-02-22 결과 보고서 고도화 + 이미지 저장 안정화
- [x] 전체 보고서 문구를 사용자 요청 충족/미충족/사유/최선안 중심으로 재작성
- [x] 개별 팀 보고서에 요청 반영 근거와 미충족 항목을 문장형으로 확장
- [x] MBTI 요청 감지 시 팀별 MBTI 조합 설명 문구 자동 생성
...
## 41. 2026-02-23 커스텀 배분 UI 직관화(+1/-1 카드 방식)
- [x] 숫자 입력칸 방식 제거
- [x] 팀 카드별 `+1/-1` 버튼 배분 방식 도입
- [x] 배분 수/남은 인원 실시간 표시 유지
- [x] 커스텀 배분 초기화 버튼 추가
- [x] 빌드 검증 (`npm run build`)

## 42. 2026-02-23 커스텀 배분 완료 상태 배지 추가
- [x] 커스텀 배분 상단에 완료/미완료 배지 표시
- [x] 남은 인원 0 기준으로 자동 상태 전환
- [x] 빌드 검증 (`npm run build`)

## 43. 2026-02-23 커스텀 배분 UI 단순화(화살표 증감 방식)
- [x] 카드형 복잡 UI를 리스트형 간결 UI로 축소
- [x] 팀별 `▲/▼` 버튼으로 +1/-1 조작 방식 통일
- [x] 상단에 남은 인원만 집중 표시
- [x] 빌드 검증 (`npm run build`)

## 44. 2026-02-23 맞춤 프롬프트 체크리스트 리포트 추가
- [x] 사용자 프롬프트를 항목별 체크리스트로 분해하는 로직 추가
- [x] 결과 리포트에 `originalPrompt`, `promptChecklist` 포함
- [x] 결과 페이지에서 접힘/펼침 형태로 원문+체크리스트 표시
- [x] 빌드 검증 (`npm run build`)

## 45. 2026-02-23 추론 2단계 분리(요청 추출 -> 배정)
- [ ] 1단계 요청 체크리스트 추출 API 함수 추가
- [ ] 2단계 배정 프롬프트를 체크리스트 입력 기반으로 전환
- [ ] 배정 엔진 파이프라인을 2단계 순차 실행으로 변경
- [ ] 리포트 체크리스트를 1단계 추출 결과 우선 사용으로 변경
- [ ] 케이스별 회귀 점검 + 빌드 검증 (`npm run build`)

## 46. 2026-02-23 팀 틀 강제/재시도 게이트 계획 반영
- [x] `targetTeamSizes` 슬롯 강제 배정 로직 적용 (팀 틀 절대 준수)
- [x] 무결성(중복/누락/invalid/팀크기) 실패 시 결과 무효 처리
- [x] 무결성 실패 조건에서만 1회 조건부 재시도
- [ ] 2차 결과 품질 열화 시 1차 결과로 자동 롤백
- [ ] 문제 구간 중심 부분 보정 + 안정 팀 잠금 전략 적용

## 47. 2026-02-26 결과 리포트 포맷 전환 (global_report 제거 + 체크리스트 상세화)
- [x] 배정 프롬프트 출력 스키마에서 `global_report/full_report/summary` 의존 제거
- [x] 체크리스트 항목별 `applied_detail`, `evidence[]` 상세 필드 강제 규칙 추가
- [x] 사전 체크리스트가 최종 체크리스트를 덮어쓰는 로직 수정 (배정 결과 체크리스트 우선)
- [x] 리포트 생성 시 summary를 체크리스트 기반으로 재구성
- [x] 결과 페이지에서 항목별 `반영 상세`, `근거` 렌더링 추가
- [x] 빌드 검증 (`npm run build`)
