...
const buildAiTeamsDirect = (aiTeams = [], memberById) => {
  if (!Array.isArray(aiTeams)) return [];

  return aiTeams.map((team, idx) => {
    const ids = Array.isArray(team?.members)
...
export const assignTeamsWithValidation = async ({
  ...
  const teams = buildAiTeamsDirect(ai?.teams, memberById);
